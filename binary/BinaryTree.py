...
class binary_tree_test(unittest.TestCase):
    def test(self):
        ...
        bt.add(7)
        print("pre order")
        bt.preorder_traverse() #why preorder? we want to keep this tree structure, sending the byte tree to another server, we can keep the structure by using preorder
        self.assertEqual(bt.preorder_list, [5,3,1,4,7])

        print("in order")#interview question? sort this binary search tree? use in order traverse, and you will sort this binary search tree.
        bt.inorder_traverse()#why in order travserse ? it gurantees a sorted order in the result.
